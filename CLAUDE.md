...
- **Кеш писем**: обработанные HTML-тела и бинарные вложения кешируются в `~/Library/Caches/MailAi/` (bodies/ и attachments/). Лимит по умолчанию 500 МБ, LRU-вытеснение по дате модификации. Пользователь может очистить кеш в Settings. В SQLite/GRDB — только метаданные (message-id, заголовки, флаги, размеры, хеши).
